...
# Each step uses the session and context established by the previous step.
...
TC_API_CRUD_01 Full Cart Item Lifecycle POST GET PUT PATCH DELETE
    [Documentation]
    ...    Complete CRUD lifecycle for a cart item:
    ...    Step 1 POST   — Add product 12345 to cart (Create).
    ...    Step 2 GET    — Fetch current cart to verify item exists (Read).
    ...    Step 3 PUT    — Fully replace the cart item with quantity=5 (Update-full).
    ...    Step 4 PATCH  — Partially update only quantity to 2 (Update-partial).
    ...    Step 5 DELETE — Remove the item from cart (Delete).
    ...
    Run Keyword    Assert No Server Error    ${post_resp}    Step 1 POST (Create)

    # Step 2 — READ (GET) — verify what was just posted
    ${get_resp}=     GET On Session    purplle    ${CART_ENDPOINT}
    ...    expected_status=any
    Run Keyword    Assert No Server Error    ${get_resp}    Step 2 GET (Read)

    # Step 3 — UPDATE full (PUT) — replace entire resource
    ...
    Run Keyword    Assert No Server Error    ${put_resp}    Step 3 PUT (Full Update)

    # Step 4 — UPDATE partial (PATCH) — only change quantity
    ...
    Run Keyword    Assert No Server Error    ${patch_resp}    Step 4 PATCH (Partial Update)

    # Step 5 — DELETE — remove the item
    ${del_resp}=     DELETE On Session    purplle    ${CART_ITEM_ENDPOINT}
    ...    expected_status=any
    Run Keyword    Assert No Server Error    ${del_resp}    Step 5 DELETE (Remove)

    Log    CRUD Chain complete:
    Log    POST=${post_resp.status_code} GET=${get_resp.status_code} PUT=${put_resp.status_code} PATCH=${patch_resp.status_code} DELETE=${del_resp.status_code}

TC_API_CRUD_02 Full Wishlist Item Lifecycle POST GET PUT PATCH DELETE
    [Documentation]
    ...    Complete CRUD lifecycle for a wishlist item:
    ...    Step 1 POST   — Add product to wishlist (Create).
    ...    Step 2 GET    — Fetch wishlist to verify item (Read).
    ...    Step 3 PUT    — Full replace of wishlist item metadata (Update-full).
    ...    Step 4 PATCH  — Partially update only priority flag (Update-partial).
    ...    Step 5 DELETE — Remove item from wishlist (Delete).
    [Tags]    api    crud    chaining    post    get    put    patch    delete    regression
...
TC_API_CRUD_03 Profile Full CRUD Cycle POST GET PUT PATCH DELETE
    [Documentation]
    ...    Complete CRUD lifecycle for a user profile (guest-level, no auth):
    ...    Step 1 POST   — Attempt to create profile data.
    ...    Step 2 GET    — Read current profile.
    ...    Step 3 PUT    — Full profile replace.
    ...    Step 4 PATCH  — Partial update (name only).
    ...    Step 5 DELETE — Attempt account deletion.
    ...    All steps expected to return 401/403 for guest — never 5xx.
    [Tags]    api    crud    chaining    post    get    put    patch    delete    regression

    # Step 1 — POST (create profile attempt)
    ...
    ...    json=${patch_body}    expected_status=any
    Run Keyword    Assert No Server Error    ${patch_resp}    Step 4 PATCH (Partial)
